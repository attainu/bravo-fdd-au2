...
function mapDispatchToProps(dispatch){
    return{  
        getSongs: function(){
            fetch("http://ws.audioscrobbler.com/2.0/?method=chart.gettoptracks&api_key=f77fce611dba3185e9c5c0cf2fcc32db&format=json&offset=1&limit=4")
            ...
            .then(result => {
                ...
                result.tracks.track.map((item, i) => { 
                    ...
                    .then(response => response.json())
                    .then(result => {
                        dispatch(getNames(result.response.hits[0].result.header_image_thumbnail_url))
                    }
                    )
                    }
                    )}
...
export default connect(mapStateToProps, mapDispatchToProps)(GlobalTop);
